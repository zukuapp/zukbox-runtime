...
이 문서는 구현 완료 목록이 아니라 형식 **초안**입니다. 현재 공개 코드는
헤더·청크와 일부 미디어 본문을 파싱하고 타임라인을 평가합니다. `SNDS` 동기,
`ATLS` 패킹, `SIGN` 서명 검증 등은 완성된 기능으로 가정하지 마세요. 구현 범위와
호스트 책임은 [README](../README.md)와 [안전 경계](safety-and-unsafe.md)에
적었습니다.

이 형식은 `ZWF1` 매직을 쓰는 ZUKBOX 바이너리입니다.
[`zwf` 저장소의 `ZWF2` HTML5 패키지](https://github.com/zukuapp/zwf/blob/main/SPEC.md)와
호환되는 형식이 아닙니다.
